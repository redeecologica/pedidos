...
<br>
  ...
  <div class="panel-heading">
     <strong>Consolidado - Distribuído para os Núcleos <?php if($prodt_nome) echo(" - " . $prodt_nome . " - " . $cha_dt_entrega); ?></strong>
...
if($cha_id!=-1)
{
	...
	{	
		?>		
                ...
                <thead>
                    <tr>
                        <th colspan="4">Consolidado Distribuído para os Núcleos <?php echo(" - " . $prodt_nome . " " .  $cha_dt_entrega); ?> </th>
                    </tr>
					<tr>
                    	<th>Núcleo</th>
                        <th>Pedido (R$)</th>
               ...
               while ($row = mysqli_fetch_array($res,MYSQLI_ASSOC)) 
               {
				   ...
				   $somatorio_recebido+=$valor_recebido;
                    ...
                    <td>
						...
						{
						?>
                            <a class="btn btn-default <?php echo($valor_distribuido>0? "" : "btn-danger" ); ?>" href="distribuicao.php?action=<?php echo(ACAO_EXIBIR_EDICAO . "&cha_id=" . $cha_id .  "&nuc_id=" . $row["nuc_id"]);?>"><i class="glyphicon glyphicon-pencil glyphicon-white"></i> atualizar</a>                            
                        
						<?php
						}
						?>
                         <a class="btn btn-default" href="distribuicao.php?action=<?php echo(ACAO_EXIBIR_LEITURA . "&cha_id=" . $cha_id .  "&nuc_id=" . $row["nuc_id"]);?>"><i class="glyphicon glyphicon-search glyphicon-white"></i> ver</a>  
                    </td>
